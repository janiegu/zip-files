...
do
	...
	topojson --id-property ZCTA5CE10 --out $(echo "topojson/0"$i".json") $(echo "geojson/0"$i".json")
    else
	...
	topojson --id-property ZCTA5CE10 --out $(echo "topojson/"$i".json") $(echo "geojson/"$i".json")
        fi
done